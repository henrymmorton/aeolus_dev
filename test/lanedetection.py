...
class Lane:
    ...
    def get_filtered(self, img=None, thresholds=None, median=False, canny=False, plot=False):
        ...
        _, b_thresholded = filter.threshold(img[:,:,0], thresholds[3])
        ...
        if median == True:
            ...
            l_blurred = filter.median_blur(l_thresholded, median_kernel = 19)
        else:
            #Apply a gaussian blur to the thresholded image to reduce noise 
            l_blurred = filter.gaussian_blur(l_thresholded, gauss_kernel = 9)
...
def pipeline(img, plotIt = False, myThresholds = None, myROI = None):
