...
def threshold(channel, thresh, thresh_type=cv2.THRESH_BINARY):
    """
...
def sobel(channel, sobel_kernel = 3, thresh=(0, 255)):
    ...
    sob_max = np.max(sobel_mag)
    sobel_scaled = sobel_mag / (sob_max / 255)
    sob_scal_max = np.max(sobel_scaled)
    
    return binary_thresh(sobel_scaled, thresh)
