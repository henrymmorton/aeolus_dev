...
NUM_CHANNELS = 3
# Number of types of things to segment the image into
NUM_CLASSES = 1
...
class MyCoTransform(object):
    def __init__(self, enc, augment=True, height=720):
        self.enc=enc
    ...
    def __call__(self, input, target):
        # Resizes the image to the correct size to be an input to the neural network
        ...
        # Resizes the object to be 1/8 as large
        if (self.enc):
            target = Resize(int(self.height/8), Image.NEAREST)(target)

        # Adds an extra 1 dimension along the label zero axis to make it have the same number of dims as the source images
...
def train(args, model, enc=False):
    ...
    # This dataset keeps the dataset as is
    co_transform_val = MyCoTransform(enc, augment=False, height=args.height)#1024)

    dataset_train = TuSimple(args.datadir, co_transform)
    dataset_val = TuSimple(args.datadir, co_transform_val)
    ...
    # This loads a partially trained model from a file
    if args.resume:
    ...
    for epoch in range(start_epoch, args.num_epochs+1):
        ...
        # Sets the internal state of the model to training rather than evaluation
        model.train()
        
        # Iterate through each batch of training examples. This loop is where each batch is trained for this epoch
        for step, (images, labels) in enumerate(loader):
            ...
            # IMPORTANT: Performs one update of all the parameters in the model based on the current gradients
            optimizer.step()

            # Add the loss value for this batch to the total loss sum for this epoch
